<template>
    <div id="jhtpcontainer">
        ...
        <div v-if="panelStatus" class="jh-time-panel">
            ...
            <div v-for="week in weekDayText" class="jh-panel-week jh-text-center jh-button-style">{{ week }}</div>
            <div v-for="day in panelPreMonth" :key="day.id" class="jh-panel-day jh-no-select jh-text-center jh-button-style">{{ day }}</div>
            <div v-for="day in panelCurMonth" class="jh-panel-day jh-text-center jh-button-style" @click="selectDay(day, $event)" v-bind:class="dayStyle(day)">{{ day }}</div>
            <div v-for="day in panelNextMonth" :key="day.id" class="jh-panel-day jh-no-select jh-text-center jh-button-style">{{ day }}</div>
    ...
    export default {
        ...
        created: function () {
            ...
            if (this.params !== undefined ) {
                if (this.params.endTime && this.params.startTime && this.params.endTime < this.params.startTime) {
                    ...
                    if (this.params.startTime) {
                        if (reg.test(this.params.startTime)) {
                            ...
                            console.warn("Please ensure your format of parameter 'startTime' like as 'YYYY-MM-DD' or 'YYYY-MM'");
                        }
                    }
                    if (this.params.endTime) {
                        if (reg.test(this.params.endTime)) {
                            this.endYearMonth = this.params.endTime.substr(0, 7);
                            this.endDay = this.params.endTime.substr(8, 2) || Moment(this.params.endTime).daysInMonth();
                        }
                        else {
                            console.warn("Please ensure your format of parameter 'endTime' like as 'YYYY-MM-DD' or 'YYYY-MM'");
                        }
                    }
                }
            }
        ...
        mounted: function () {
            ...
            document.getElementById('jhtpcontainer').addEventListener('click', function (e) {
                e.stopPropagation();
            });
        },
        methods: {
            ...
            createCurMonth (time) {
                ...
                let startDay = time.substr(0,7) !== this.startYearMonth? 1 : parseInt(this.startDay);
                let endDay = time.substr(0,7) !== this.endYearMonth ? preLastDay : this.endDay;
            ...
            goPreMonth () {
                let moment = Moment(this.panelTime);
            ...
            dayStyle (day) {
                if (day === Moment(this.realTime).date() && this.realTime.substr(0,7) === this.panelTime) {
                    if (this.realTime === this.pickTime) {
                        return "jh-today-style jh-select-style"
                    }
                    return "jh-today-style"
                }
                if (day === Moment(this.pickTime).date() && this.pickTime.substr(0,7) === this.panelTime) {
